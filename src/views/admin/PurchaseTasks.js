...
import { cilClipboard, cilFilter, cilList, cilPeople, cilSpeedometer, cilPencil } from '@coreui/icons'
...
const PurchaseTasks = () => {
  ...
  }, [activeTab, statusFilter, adminStatusFilter, adminRoleFilter, adminEmployeeFilter])

  const handleStatusChange = async (task, newStatus) => {
    if (!newStatus || newStatus === task.status) return
    try {
      await purchaseTaskService.updateStatus(task._id || task.id, newStatus, task.targetRate)
  ...
  const handleSaveTargetRate = async () => {
    ...
    try {
      ...
      await purchaseTaskService.updateStatus(task._id || task.id, task.status || '', value)
  ...
  const renderTaskRow = (task, index, showAssignee = false) => {
    ...
    return (
      <CTableRow key={task._id || task.id || index}>
          ...
          <strong>
            {quotation.quotationCode ||
              (quotation._id || quotation.id
                ? `QT-${String(quotation._id || quotation.id).slice(-4).padStart(4, '0')}`
                : '-')}
          </strong>
        </CTableDataCell>
        <CTableDataCell>
        ...
        <CTableDataCell style={{ verticalAlign: 'middle', whiteSpace: 'nowrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div
              style={{
                padding: '6px 10px',
                borderRadius: 6,
                background: task.targetRate != null ? 'rgba(16, 185, 129, 0.08)' : 'transparent',
            ...
            {(user?.role === ROLES.PURCHASE_MANAGER || user?.role === ROLES.PURCHASE_EXICUTIVE) && (
              <CButton
                color="secondary"
                variant="ghost"
                size="sm"
        ...
        {showAssignee && (
          <CTableDataCell>
            ...
            {task.assignedTo?.designation && (
              <div className="small text-muted">{task.assignedTo.designation}</div>
            )}
          </CTableDataCell>
        )}
        <CTableDataCell>
      ...
      <CTableBody>
        {items && items.length > 0 ? (
          items.map((task, index) => renderTaskRow(task, index, showAssignee))
            ...
            <CTableRow>
            <CTableDataCell colSpan={showAssignee ? 12 : 11} className="text-center">
              No tasks found.
            </CTableDataCell>
          </CTableRow>
        )}
  ...
  return (
    <>
        ...
        <CCol className="d-flex justify-content-between align-items-center">
          <div>
            ...
            <p className="text-muted mb-0">
              Manage quotation-based purchase tasks, rate bucket, and admin tracking.
            </p>
          </div>
        ...
        <CCardBody>
          <CTabContent>
            <CTabPane visible={activeTab === 'purchase'}>
              ...
              {loading ? <Loader message="Loading tasks..." /> : renderTasksTable(tasks)}
            </CTabPane>
            ...
            <CTabPane visible={activeTab === 'admin'}>
              {isAdminLike ? (
                ...
                <div className="text-muted">You do not have access to the admin view.</div>
              )}
            </CTabPane>
          </CTabContent>
        </CCardBody>
      ...
      <CModal visible={!!remarkModalTask} onClose={closeRemarkModal}>
        ...
        <CModalFooter>
          <CButton color="secondary" variant="outline" onClick={closeRemarkModal}>
            Cancel
          </CButton>
          <CButton color="primary" onClick={handleSaveRemark} disabled={savingRemark}>
      ...
      <CModal visible={!!targetRateModalTask} onClose={closeTargetRateModal}>
        ...
        <CModalFooter>
          <CButton color="secondary" variant="outline" onClick={closeTargetRateModal}>
            Cancel
          </CButton>
          <CButton color="primary" onClick={handleSaveTargetRate} disabled={savingTargetRate}>
