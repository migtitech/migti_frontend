...
const SubZoneList = () => {
  ...
  const [editModal, setEditModal] = useState({ visible: false, subZoneId: '', name: '' })
  ...
  return (
    <>
              ...
              {zones.map((zone) => {
                ...
                return (
                  <CAccordionItem itemKey={String(zid)} key={String(zid)}>
                    <CAccordionHeader>
                      {label}
                      <span className="text-muted small ms-2">({subs.length} sub-zone{subs.length === 1 ? '' : 's'})</span>
                    </CAccordionHeader>
                    <CAccordionBody>
                      {subs.length === 0 ? (
                        <p className="text-muted small mb-0">No sub-zones for this zone.</p>
                      ) : (
                                ...
                                <CTableDataCell className="text-end">
                                  ...
                                  {canDelete('zones') && (
                                    <CButton
                                      ...
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setConfirmDelete({ visible: true, id: getId(sz) })}
                                    >
                                      <CIcon icon={cilTrash} />
                                    </CButton>
                                  )}
      ...
      <CModal visible={editModal.visible} onClose={() => setEditModal({ visible: false, subZoneId: '', name: '' })}>
        <CModalHeader>
        ...
        <CModalFooter>
          <CButton color="secondary" variant="outline" onClick={() => setEditModal({ visible: false, subZoneId: '', name: '' })}>
            Cancel
          </CButton>
