...
const EmployeePermissionsSection = ({ selectedRole, permissions = [], onChange }) => {
  ...
  const selectAll = () => {
    const allPerms = modules.flatMap((m) => ACTIONS.map((a) => `${m.key}:${a}`))
    onChange(allPerms)
  }
  ...
  const allSelected = modules.length > 0 &&
    modules.every((m) => ACTIONS.every((a) => permissions.includes(`${m.key}:${a}`)))
            ...
            {ACTIONS.map((action) => (
              <CTableHeaderCell key={action} className="text-center">
                <CFormCheck
                  ...
                  checked={modules.length > 0 && modules.every((m) => permissions.includes(`${m.key}:${action}`))}
