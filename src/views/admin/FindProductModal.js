...
const FindProductModal = ({ visible, onClose, onImport }) => {
  ...
  const [selectedVariantOptions, setSelectedVariantOptions] = useState(new Map())
  ...
  const fetchSubcategoryOptions = useCallback(
    async (term, catId, isOpen) => {
      if (!catId || !isOpen) {
        setSubcategoryResults([])
        return
      }
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedIds((prev) => {
      ...
      if (allSelected) return new Set([...prev].filter((id) => !productIds.includes(id)))
          ...
          <CRow className="g-3 mb-3">
            <CCol md={4}>
              <div className="position-relative">
                ...
                />
                {categoryId && (
                  <CButton color="link" size="sm" className="p-0 mt-1" onClick={handleClearCategory}>
                    Clear
                  </CButton>
                )}
                {categoryDropdownOpen && (categoryResults?.length > 0 || categoryLoading) && (
                  <div
            ...
            <CCol md={4}>
              <div className="position-relative">
                ...
                <CFormInput
                  ...
                  onBlur={() => setTimeout(() => setSubcategoryDropdownOpen(false), 200)}
                  placeholder={categoryId ? 'Type to search subcategory...' : 'Select category first'}
                  autoComplete="off"
                  disabled={!categoryId}
                />
                {subcategoryId && (
                  <CButton color="link" size="sm" className="p-0 mt-1" onClick={handleClearSubcategory}>
                    Clear
                  </CButton>
                )}
                {subcategoryDropdownOpen && categoryId && (subcategoryResults?.length > 0 || subcategoryLoading) && (
                  <div
                    className="position-absolute w-100 bg-white border rounded mt-1 shadow-sm"
                    style={{ zIndex: 1050, maxHeight: 200, overflowY: 'auto' }}
                  >
            ...
            <CCol md={4}>
              <div className="position-relative">
                ...
                <CFormInput
                  ...
                  autoComplete="off"
                />
                {productDropdownOpen && (productResults?.length > 0 || productLoading) && (
                  <div
                    className="position-absolute w-100 bg-white border rounded mt-1 shadow-sm"
                    style={{ zIndex: 1050, maxHeight: 200, overflowY: 'auto' }}
                  >
                      ...
                      {!productLoading &&
                        productResults.map((p) => (
                          ...
                          >
                            ...
                            {p.sku && <div className="text-muted small">SKU: {p.sku}</div>}
                          </CListGroupItem>
                        ))}
                    </CListGroup>
                  </div>
        ...
        {products.length > 0 && (
          <>
            <div className="d-flex justify-content-between align-items-center mb-2">
              <strong>
                {products.length} product(s) — check the items you want, then click Import to fill form
              </strong>
              <CFormCheck
                type="checkbox"
                id="selectAll"
            ...
            <div className="table-responsive" style={{ maxHeight: 400, overflowY: 'auto' }}>
              {products.map((p) => {
                  ...
                  return (
                    <CCard key={pid} className="mb-3">
                      ...
                      <CCardBody className="py-2">
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          <CFormLabel className="mb-0">Variant combinations — select to import</CFormLabel>
                          <CFormCheck
                            type="checkbox"
                            label="Select all"
                            checked={allSelected}
                              ...
                              return (
                                <CTableRow key={uid}>
                                  <CTableDataCell>
                                    <CFormCheck
                                      checked={checked}
                                      onChange={() => toggleVariantCombo(pid, uid)}
                                    />
                                  </CTableDataCell>
                ...
                if (hasVariantOpts) {
                  const optionSet = selectedVariantOptions.get(pid) || new Set()
                  ...
                  return (
                    <CCard key={pid} className="mb-3">
                      ...
                      <CCardBody className="py-2">
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          <CFormLabel className="mb-0">Variants — select to import</CFormLabel>
                          <CFormCheck
                            type="checkbox"
                            label="Select all"
                            checked={allSelected}
                            onChange={() => toggleAllVariantOptions(pid, variantOpts)}
                          />
                        </div>
                        <CTable size="sm" hover>
                          <CTableHead>
                              ...
                              return (
                                <CTableRow key={o.key}>
                                  <CTableDataCell>
                                    <CFormCheck
                                      checked={checked}
                                      onChange={() => toggleVariantOption(pid, o.key)}
                                    />
                                  </CTableDataCell>
