...
import { cilArrowLeft, cilArrowRight, cilCloudDownload, cilX } from '@coreui/icons'
...
const PurchaseOrderView = () => {
  ...
  const [salesEmployeesForPo, setSalesEmployeesForPo] = useState([])
  const [salesAssignSelection, setSalesAssignSelection] = useState('')
  const [assigningSalesPerson, setAssigningSalesPerson] = useState(false)
  const [paymentAmountInput, setPaymentAmountInput] = useState('')
  const [paymentDateInput, setPaymentDateInput] = useState(() => todayLocalYmd())
  const [paymentRemarkInput, setPaymentRemarkInput] = useState('')
  const [appendingPayment, setAppendingPayment] = useState(false)
  ...
  useEffect(() => {
    ...
    const raw = se && typeof se === 'object' ? (se._id ?? se.id) : se
    setSalesAssignSelection(raw ? String(raw) : '')
  }, [purchaseOrder?._id, purchaseOrder?.id, purchaseOrder?.salesEmployeeId])

  const companyInfo = purchaseOrder?.companyInfo || null
  const products = Array.isArray(purchaseOrder?.products) ? purchaseOrder.products : []
  const branchSignature = purchaseOrder?.branchSignature || null
  ...
  const branchSignaturePath =
    branchSignature && typeof branchSignature === 'object' && branchSignature.path
      ...
      : ''
  const queryId = purchaseOrder?.queryId?._id ?? purchaseOrder?.queryId
  const relatedQueryCode =
    purchaseOrder?.queryId && typeof purchaseOrder.queryId === 'object' && purchaseOrder.queryId.queryCode
      ? String(purchaseOrder.queryId.queryCode).trim()
      : ''
  const relatedQueryLinkLabel = relatedQueryCode || 'View Query'
  const sourceQuotationId =
    purchaseOrder?.quotationId && typeof purchaseOrder.quotationId === 'object'
      ? purchaseOrder.quotationId._id ?? purchaseOrder.quotationId.id
      : purchaseOrder?.quotationId
  const sourceQuotationCode =
    purchaseOrder?.quotationId &&
    typeof purchaseOrder.quotationId === 'object' &&
    purchaseOrder.quotationId.quotationCode
      ? String(purchaseOrder.quotationId.quotationCode).trim()
      : ''

  useEffect(() => {
    const ci = purchaseOrder?.companyInfo
    if (ci) {
  ...
  useEffect(() => {
    if (purchaseOrder) {
      const freight = purchaseOrder.freightCharge
      const packing = purchaseOrder.packingCharge
      const expWithinDays = purchaseOrder.expectedDeliveryWithinDays
  ...
  const handleSavePackingDelivery = async () => {
    ...
    try {
      const res = await purchaseOrderService.update(purchaseOrder.id, {
  ...
  const handleSaveCompanyInfo = async () => {
    ...
    try {
      ...
      const data = res?.data?.data ?? res?.data ?? res
      if (data?.companyInfo) {
        setPurchaseOrder((prev) => (prev ? { ...prev, companyInfo: data.companyInfo } : null))
      } else {
        setPurchaseOrder((prev) => (prev ? { ...prev, companyInfo: { name: companyForm.name, location: companyForm.location, area: companyForm.area, address: companyForm.address, purchaseManagers } } : null))
      }
  ...
  const handleAssignSalesPerson = async () => {
    ...
    try {
      const res = await purchaseOrderService.update(purchaseOrder.id, {
        salesEmployeeId: salesAssignSelection || null,
      })
      const data = res?.data?.data ?? res?.data ?? res
      if (data) {
        setPurchaseOrder((prev) => {
          ...
          return {
            ...prev,
            salesEmployeeId: data.salesEmployeeId !== undefined ? data.salesEmployeeId : prev.salesEmployeeId,
            financials: data.financials ?? prev.financials,
          }
        })
      }
      toastSuccess('Sales person assigned')
    } catch (err) {
      toastError(err?.response?.data?.message || err?.message || 'Failed to assign sales person')
    } finally {
  ...
  const handleAppendPayment = async () => {
    ...
    if (paymentAmountInput === '' || Number.isNaN(amt) || amt <= 0) {
      toastError('Enter a valid payment amount')
      return
    }
    setAppendingPayment(true)
    try {
      const paidAt =
        paymentDateInput && paymentDateInput.trim()
          ? new Date(`${paymentDateInput.trim()}T12:00:00`).toISOString()
          : undefined
      const res = await purchaseOrderService.appendPayment(purchaseOrder.id, {
        amount: amt,
        paidAt,
        remark: paymentRemarkInput || '',
      })
      ...
      if (po) {
        setPurchaseOrder((prev) => {
          ...
          return {
            ...prev,
            payments: Array.isArray(po.payments) ? po.payments : prev.payments,
            financials: fin ?? prev.financials,
          }
        })
      }
      setPaymentAmountInput('')
      setPaymentRemarkInput('')
      setPaymentDateInput(todayLocalYmd())
      toastSuccess('Payment recorded')
    } catch (err) {
      toastError(err?.response?.data?.message || err?.message || 'Failed to record payment')
    } finally {
      setAppendingPayment(false)
    }
  }
  ...
  useEffect(() => {
    if (activeTab !== 'salesInfo') return
    let cancelled = false
    employeeService
      .getAll({ pageNumber: 1, pageSize: 100, rolePrefix: 'sales' })
      .then((res) => {
        if (cancelled) return
        const data = res?.data || res
        const result = data?.data ?? data
        const list = result?.employees || result?.items || result || []
        setSalesEmployeesForPo(Array.isArray(list) ? list : [])
      })
      .catch(() => {
        if (!cancelled) setSalesEmployeesForPo([])
      })
    return () => { cancelled = true }
  }, [activeTab])
  ...
  const openAssignTaskModal = () => {
    const selectedIds = Object.entries(assignTaskSelected)
      ...
      .map(([id]) => id)
    if (!purchaseOrder?.id || selectedIds.length === 0) {
  ...
  const handleAssignTask = async () => {
    const selectedIds = Object.entries(assignTaskSelected)
      .filter(([, checked]) => checked)
      .map(([id]) => id)
    if (!purchaseOrder?.id || selectedIds.length === 0) return
    const p = products[Math.min(productIndex, products.length - 1)]
    const productRef = typeof p?.product_id === 'object' ? p.product_id : null
    const productObj = {
    ...
    const quotationNumber = sourceQuotationCode || `QT-${String(sourceQuotationId || '').slice(-6)}` || ''
    if (!sourceQuotationId) {
      toastError('No source quotation is linked; tasks must reference a quotation.')
      return
    }
  ...
  const handleUpdateProduct = async () => {
    if (!purchaseOrder?.id || !editingProduct) return false
    const idx = Math.min(productIndex, products.length - 1)
    ...
    setUpdating(true)
    try {
      const res = await purchaseOrderService.update(purchaseOrder.id, { products: updatedProducts })
      const data = res?.data?.data ?? res?.data ?? res
      if (data?.products) {
        setPurchaseOrder((prev) => (prev ? { ...prev, products: data.products } : null))
      } else {
        setPurchaseOrder((prev) => (prev ? { ...prev, products: updatedProducts } : null))
      }
  ...
  const poFinancials = purchaseOrder?.financials
  const formatPoInr = (n) =>
    typeof n === 'number' && !Number.isNaN(n)
      ? `₹${n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      : '–'
  const grandTotalNum = Number(poFinancials?.grandTotal)
  const totalPaidNum = Number(poFinancials?.totalPaid)
  const paymentFinancialsComparable =
    poFinancials != null &&
    !Number.isNaN(grandTotalNum) &&
    !Number.isNaN(totalPaidNum)
  const parsedAddPaymentAmount =
    paymentAmountInput === '' || String(paymentAmountInput).trim() === ''
      ? NaN
      : Number(paymentAmountInput)
  const maxAdditionalPayment = paymentFinancialsComparable
    ? Math.max(0, Number((grandTotalNum - totalPaidNum).toFixed(2)))
    : null
  const paymentExceedsGrandTotal =
    paymentFinancialsComparable &&
    !Number.isNaN(parsedAddPaymentAmount) &&
    parsedAddPaymentAmount > 0 &&
    Number((totalPaidNum + parsedAddPaymentAmount).toFixed(2)) > Number(grandTotalNum.toFixed(2))
  const paymentAmountErrorMessage = paymentExceedsGrandTotal
    ? totalPaidNum >= grandTotalNum
      ? 'Total paid already equals or exceeds grand total. You cannot add more.'
      : `Total paid cannot exceed grand total. Maximum you can add is ${formatPoInr(maxAdditionalPayment)}.`
    : ''
  const paymentLedgerRows = (() => {
    const list = Array.isArray(purchaseOrder?.payments) ? [...purchaseOrder.payments] : []
    return list.sort((a, b) => new Date(b.paidAt || 0) - new Date(a.paidAt || 0))
  })()
  const assignedSalesPerson = purchaseOrder?.salesEmployeeId
  const assignedSalesPersonLabel =
    assignedSalesPerson && typeof assignedSalesPerson === 'object'
      ? [assignedSalesPerson.name, assignedSalesPerson.role].filter(Boolean).join(' · ') || '–'
      : null
  ...
  const handleUpdateProductFromList = async (idx) => {
    if (!purchaseOrder?.id || !products[idx]) return
    const p = products[idx]
  ...
  const handleConfirmDeleteProduct = async () => {
    if (!purchaseOrder?.id || deleteProductIndex == null) return
    ...
    try {
      ...
      setPurchaseOrder((prev) => (prev ? { ...prev, products: nextProducts } : null))
      setDeleteProductModalVisible(false)
      setDeleteProductIndex(null)
      toastSuccess('Product deleted from purchase order')
  ...
  const handleRevokeNotAvailable = async (idx) => {
    if (idx == null || !purchaseOrder?.id || !products[idx]) return
    const p = products[idx]
  ...
  const handleProductListAssignTask = async () => {
    ...
    if (!purchaseOrder?.id || selectedIndices.length === 0 || !productListAssignEmployeeId) {
  ...
  const handleAddNewProduct = async () => {
    ...
    try {
      ...
      clearNewProductForm()
      toastSuccess('Product added to purchase order')
  ...
  const handleDownloadProductsPdf = async () => {
    if (!sourceQuotationId) {
      toastError('No linked quotation — PDF uses the source quotation.')
      return
    }
    setExportingPdf(true)
    try {
      const response = await quotationService.exportPdf(sourceQuotationId)
      ...
      a.href = url
      a.download = `quotation-${sourceQuotationCode || sourceQuotationId}-${new Date().toISOString().slice(0, 10)}.pdf`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      toastSuccess('Quotation PDF downloaded')
    } catch (err) {
  ...
  const isHodApproved =
    Boolean(sourceQuotationId) &&
    purchaseOrder?.quotationId &&
    typeof purchaseOrder.quotationId === 'object' &&
    purchaseOrder.quotationId.status === 'hod_approved'
  ...
  const isPoConfirmed = purchaseOrder?.status === 'confirmed'

  const handleMarkApproved = async () => {
    if (!purchaseOrder?.id) return
    try {
      await purchaseOrderService.updateStatus(purchaseOrder.id, 'confirmed')
      setPurchaseOrder((prev) => (prev ? { ...prev, status: 'confirmed' } : null))
      toastSuccess('Purchase order marked as confirmed')
    } catch (err) {
    ...
    <>
      <CCard className="mb-4 border-0 shadow-sm" style={{ borderRadius: 12, backgroundColor: '#f8f9fb' }}>
        <CCardBody className="p-3 p-md-4">
          {/* Row 1: PO number only */}
          <div className="mb-3">
            <div
              className="fw-bold text-primary px-3 py-2 rounded-pill border d-inline-block"
              style={{ fontSize: '1rem', letterSpacing: '0.3px', backgroundColor: '#eef4ff' }}
            >
              {purchaseOrder.poCode || `PO-${String(purchaseOrder.id).slice(-6)}`}
            </div>
          </div>
          ...
          >
            ...
            <div className="d-flex flex-wrap justify-content-md-end align-items-center gap-2">
              ...
              <CButton
                ...
                title={
                  !sourceQuotationId
                    ? 'Requires a linked quotation'
                    : !isHodApproved
                      ? 'Available after the source quotation is HOD approved'
                      : undefined
                }
                className="d-inline-flex align-items-center px-3"
                style={{ height: 40 }}
              >
                ...
                {exportingPdf ? 'Generating PDF...' : 'Download quotation PDF'}
              </CButton>
            </div>
          </div>
        </CCardBody>
      ...
      <CCard className="mb-4">
        <CCardHeader className="border-bottom" style={{ backgroundColor: '#fbfcfe' }}>
          <CNav variant="tabs" role="tablist" className="gap-2">
            ...
            <CNavItem>
              <CNavLink
                active={activeTab === 'salesInfo'}
                onClick={() => setActiveTab('salesInfo')}
                style={{
                  cursor: 'pointer',
                  border: 'none',
                  borderBottom: activeTab === 'salesInfo' ? '2px solid #321fdb' : '2px solid transparent',
                  color: activeTab === 'salesInfo' ? '#321fdb' : '#6c757d',
                  fontWeight: 600,
                  paddingInline: 10,
                }}
              >
            ...
            <CNavItem>
              <CNavLink
                active={activeTab === 'paymentTrail'}
                onClick={() => setActiveTab('paymentTrail')}
                style={{
                  cursor: 'pointer',
                  border: 'none',
                  borderBottom: activeTab === 'paymentTrail' ? '2px solid #321fdb' : '2px solid transparent',
                  color: activeTab === 'paymentTrail' ? '#321fdb' : '#6c757d',
                  fontWeight: 600,
                  paddingInline: 10,
                }}
              >
        ...
        <CCardBody>
          <CTabContent>
            {/* Tab 0: Preview - read-only full purchase order */}
            <CTabPane visible={activeTab === 'preview'}>
              <CCard className="mb-4">
                <CCardHeader className="bg-light">
                  <strong>Purchase order preview</strong> — matches quotation PDF layout (read-only)
                </CCardHeader>
                <CCardBody>
                          ...
                          <CTableBody>
                            ...
                            <CTableRow>
                              ...
                              <CTableDataCell style={{ whiteSpace: 'pre-wrap' }}>
                                {companyForm.address ||
                                  purchaseOrder?.companyInfo?.address ||
                                  '–'}
                              </CTableDataCell>
                            </CTableRow>
                            <CTableRow>
                              ...
                              <CTableDataCell>
                                {companyForm.purchaseManagerName ||
                                  purchaseOrder?.companyInfo?.purchaseManagers?.[0]?.name ||
                                  purchaseOrder?.industry_id?.purchase_manager_name ||
                                  '–'}
                              </CTableDataCell>
                            </CTableRow>
                            <CTableRow>
                              ...
                              <CTableDataCell>
                                {companyForm.purchaseManagerPhone ||
                                  purchaseOrder?.companyInfo?.purchaseManagers?.[0]?.phone ||
                                  purchaseOrder?.industry_id?.purchase_manager_phone ||
                                  '–'}
                              </CTableDataCell>
                            </CTableRow>
                            <CTableRow>
                              ...
                              <CTableDataCell>
                                {companyForm.purchaseManagerEmail ||
                                  purchaseOrder?.companyInfo?.purchaseManagers?.[0]?.email ||
                                  purchaseOrder?.companyInfo?.email ||
                                  purchaseOrder?.industry_id?.email ||
                        ...
                        <div className="mt-2 pt-2 border-top small">
                          <div>
                            ...
                            {purchaseOrder?.poCode ||
                              `PO-${String(purchaseOrder?.id || '').slice(-6)}` ||
                              '–'}
                          </div>
                          <div className="mt-1">
                            <span className="fw-semibold">Quotation:</span>{' '}
                            {sourceQuotationId ? (
                              <CButton
                                color="link"
                                className="p-0 align-baseline"
                                onClick={() => navigate(`/quotations/${sourceQuotationId}`)}
                              >
                                {sourceQuotationCode || 'View quotation'}
                              </CButton>
                            ) : (
                              '–'
                            )}
                  ...
                  {/* Line items (mirror quotation PDF columns) */}
                  ...
                  <div className="d-flex flex-column flex-lg-row justify-content-between align-items-start gap-4">
                    <div style={{ flex: '0 0 55%' }}>
                      ...
                      <ul className="mb-0 small ps-3">
                        <li>Validity – Offer valid for 2 days from purchaseOrder date.</li>
                      ...
                      <CTable bordered size="sm" className="mb-3">
                        <CTableBody>
                          ...
                          <CTableRow>
                            ...
                            <CTableDataCell className="text-end">
                              ₹
                              {(Number(purchaseOrder?.freightCharge) || 0).toLocaleString(
                                undefined,
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                          ...
                          <CTableRow>
                            ...
                            <CTableDataCell className="text-end">
                              ₹
                              {(Number(purchaseOrder?.packingCharge) || 0).toLocaleString(
                                undefined,
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                          ...
                          <CTableRow>
                            ...
                            <CTableDataCell className="text-end">
                              {purchaseOrder?.expectedDeliveryWithinDays != null &&
                              !Number.isNaN(Number(purchaseOrder.expectedDeliveryWithinDays))
                                ? `${Number(purchaseOrder.expectedDeliveryWithinDays)} Days`
                                : 'NA'}
                            </CTableDataCell>
                          </CTableRow>
                          <CTableRow>
                            ...
                            <CTableDataCell className="text-end">
                              {(() => {
                                const freight =
                                  Number(purchaseOrder?.freightCharge) || 0
                                const packing =
                                  Number(purchaseOrder?.packingCharge) || 0
                ...
                <CCardBody>
                  {queryId ? (
                    <p className="mb-0 text-muted">
                      {relatedQueryCode
                        ? `This purchase order is linked to query ${relatedQueryCode}. Click the query number above to open it.`
                        : 'This purchase order is linked to a query. Click the link above to open it.'}
                    </p>
                  ) : (
                  ...
                  {purchaseOrder.remark && (
                    <div className="mt-3 pt-3 border-top">
                      ...
                      <div className="mt-1">{purchaseOrder.remark}</div>
                    </div>
                  )}
                </CCardBody>
              </CCard>
              ...
              <CCard className="mb-4">
                ...
                <CCardBody>
                  {products.length === 0 || !editingProduct ? (
                    <p className="text-muted mb-0">No products in this purchase order.</p>
                  ) : (
                    <>
                      {isCurrentProductNotAvailable && (
                        <div className="alert alert-warning py-2 px-3 mb-3">
                          This product is marked as not available. Use <strong>Revoke</strong> in the Product
                          List tab to edit it again.
                        </div>
                      )}
                      <CRow>
                        <CCol md={4}>
            ...
            <CTabPane visible={activeTab === 'salesInfo'}>
              <CCard className="mb-0">
                <CCardHeader className="bg-light">
                ...
                <CCardBody>
                  {assignedSalesPersonLabel ? (
                    <p className="text-muted small mb-3">
                      Currently assigned: <strong className="text-body">{assignedSalesPersonLabel}</strong>
                    </p>
                  ) : (
                    <p className="text-muted small mb-3">No sales person assigned yet.</p>
                  )}
                  <div className="mb-3" style={{ maxWidth: 420 }}>
                    <CFormLabel>Sales person (role starts with &quot;sales&quot;)</CFormLabel>
                    <CFormSelect
                      value={salesAssignSelection}
                      onChange={(e) => setSalesAssignSelection(e.target.value)}
                    >
                      <option value="">— Unassign —</option>
                      {salesEmployeesForPo.map((emp) => (
                  ...
                  <CButton color="primary" onClick={handleAssignSalesPerson} disabled={assigningSalesPerson || !purchaseOrder?.id}>
                    {assigningSalesPerson ? <><CSpinner size="sm" className="me-2" />Saving...</> : 'Assign'}
                  </CButton>
                </CCardBody>
              </CCard>
            </CTabPane>

            <CTabPane visible={activeTab === 'paymentTrail'}>
              <CCard className="mb-4">
                <CCardHeader className="bg-light">
                ...
                <CCardBody>
                  <CRow className="g-3">
                    <CCol sm={6} md={4}>
                      ...
                      <div className="fw-semibold">{formatPoInr(poFinancials?.grandTotal)}</div>
                    </CCol>
                    <CCol sm={6} md={4}>
                      <div className="small text-muted">Total paid</div>
                      <div className="fw-semibold">{formatPoInr(poFinancials?.totalPaid)}</div>
                    </CCol>
                    <CCol sm={6} md={4}>
                      <div className="small text-muted">Remaining</div>
                      <div className="fw-semibold text-danger">{formatPoInr(poFinancials?.remainingAmount)}</div>
                    </CCol>
                  </CRow>
                </CCardBody>
              </CCard>
                  ...
                  <CRow className="g-3 align-items-end">
                    <CCol sm={6} md={4}>
                      ...
                      <CFormInput
                        ...
                        invalid={paymentExceedsGrandTotal}
                        onChange={(e) => setPaymentAmountInput(e.target.value)}
                      />
                      <CFormFeedback invalid={paymentExceedsGrandTotal}>
                    ...
                    <CCol xs={12} md={4}>
                      <CButton
                        ...
                        disabled={
                          appendingPayment ||
                          !purchaseOrder?.id ||
                          paymentExceedsGrandTotal
                        }
                      >
                        {appendingPayment ? <><CSpinner size="sm" className="me-2" />Saving...</> : 'Add payment'}
                      </CButton>
                    </CCol>
                    <CCol xs={12}>
                      <CFormLabel>Remark (optional)</CFormLabel>
                      <CFormInput
                        value={paymentRemarkInput}
                        onChange={(e) => setPaymentRemarkInput(e.target.value)}
                        placeholder="Note"
                      />
                    </CCol>
                  </CRow>
                      ...
                      <CTableBody>
                        {paymentLedgerRows.map((row, idx) => (
                          <CTableRow key={idx}>
                            ...
                            <CTableDataCell className="text-end fw-semibold">
                              {typeof row.amount === 'number' || (row.amount != null && !Number.isNaN(Number(row.amount)))
                            ...
                            <CTableDataCell className="text-muted small">{row.remark || '—'}</CTableDataCell>
                          </CTableRow>
                        ))}
                      </CTableBody>
                    </CTable>
                  ) : (
                    <p className="text-muted mb-0 p-3">No payments recorded yet.</p>
                  )}
                </CCardBody>
              </CCard>
            </CTabPane>
      ...
      >
        ...
        <CModalBody>
          Are you sure you want to delete this product from the purchase order?
        </CModalBody>
        <CModalFooter>
          <CButton
            color="secondary"
