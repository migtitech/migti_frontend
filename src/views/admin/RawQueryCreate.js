...
useEffect(() => {
  const fetchRawQuery = async () => {
    ...
    try {
      const res = await rawQueryService.getById(id)
      const data = res?.data || res
      ...
      if (data.files?.length > 0) {
        const formattedFiles = data.files.map((file, index) => ({
          id: `existing-${index}`,
          name: `voice-${index}.webm`,
          url: file,
          dataUrl: file,
          durationSec: 0,
        }))

        setAudioClips(formattedFiles)
      }

    } catch (err) {
      toastError('Failed to load raw query')
    }
  }
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const files = audioClips.map((clip) => clip.dataUrl).filter(Boolean)
     ...
     if (isEditMode) {
      await rawQueryService.update(id, {
        ...formData,
        industryId,
        files,
      })
    ...
    toastError(err?.message || (isEditMode ? 'Failed to create raw query'
      : 'Failed to update raw query'))
