...
const BACK_PATH = "/purchase-request-bucket";
...
/**
 * Purchases Request — full detail page. Shows the entire story of a request so
 ...
 * top-right Action button opens the Assign-to-Local / Other-Purchase menu.
 * Frontend-only sample data.
 */
const PurchaseRequestBucketDetail = () => {
  const { id } = useParams();
  ...
  if (data.notFound) {
    return (
      <div className="space-y-6">
        <PageHeader title={`Purchases Request ${id}`} back={BACK_PATH} />
        <DetailNotFound label="Purchases Request" id={id} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Purchases Request · ${data.id}`}
...
export default PurchaseRequestBucketDetail;
