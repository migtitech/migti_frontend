...
const SalesDashboard = () => {
      ...
      try {
        ...
        if (cardsResult.status === 'fulfilled') {
          ...
          setDashboard({
            ...
            monthlyPurchaseOrdersCount: Number(data.monthlyPurchaseOrdersCount || 0),
            pendingQueriesCount: Number(data.pendingQueriesCount || 0),
            pendingQuotationsCount: Number(data.pendingQuotationsCount || 0),
            pendingPurchaseOrdersCount: Number(data.pendingPurchaseOrdersCount || 0),
            pendingCollectionAmount: Number(data.pendingCollectionAmount || 0),
            pendingQueries: Array.isArray(data.pendingQueries) ? data.pendingQueries : [],
            pendingQuotations: Array.isArray(data.pendingQuotations) ? data.pendingQuotations : [],
  ...
  return (
    <>
      <CRow className="mb-4">
        <CCol>
          <div className="text-body-secondary small mb-2">{formatTodayHeading()}</div>
            ...
            <CCol xs={6}>
              <div className="mb-3">
                <CWidgetStatsA
                  ...
                  chart={<CIcon icon={cilNotes} height={44} className="my-3 text-white opacity-25" />}
                />
                <div className="small text-body-secondary mt-1 px-1">Draft — action needed</div>
              </div>
            </CCol>
            <CCol xs={6}>
              <div className="mb-3">
                <CWidgetStatsA
                  ...
                  chart={<CIcon icon={cilCart} height={44} className="my-3 text-white opacity-25" />}
                />
                <div className="small text-body-secondary mt-1 px-1">Not closed</div>
              </div>
            </CCol>
            <CCol xs={6}>
              <div className="mb-3">
                <CWidgetStatsA
                  ...
                  title="Pending POs"
                  chart={<CIcon icon={cilBasket} height={44} className="my-3 text-white opacity-25" />}
                />
                <div className="small text-body-secondary mt-1 px-1">Open orders</div>
              </div>
            </CCol>
            <CCol xs={6}>
              <div className="mb-3">
                <CWidgetStatsA
                  ...
                  chart={<CIcon icon={cilDollar} height={44} className="my-3 text-white opacity-25" />}
                />
                <div className="small text-body-secondary mt-1 px-1">To collect</div>
              </div>
            </CCol>
          </CRow>
            ...
            <CCol lg={12} className="mb-4">
              <CCard>
                <CCardHeader>
                  ...
                  <small className="text-body-secondary ms-2">Draft — not yet converted</small>
                </CCardHeader>
                <CCardBody>
                  <CTable hover responsive>
                    ...
                    <CTableBody>
                      {dashboard.pendingQueries.length === 0 ? (
                        <CTableRow>
                          <CTableDataCell colSpan={5} className="text-body-secondary text-center py-4">
                            No pending queries
                          </CTableDataCell>
                        </CTableRow>
                      ) : (
                        dashboard.pendingQueries.map((row) => (
                          <CTableRow key={row.id}>
                            <CTableDataCell>{row.queryCode || '—'}</CTableDataCell>
                            <CTableDataCell>{row.companyName || '—'}</CTableDataCell>
                            <CTableDataCell>{formatQueryStatus(row.status)}</CTableDataCell>
            ...
            <CCol lg={12} className="mb-4">
              <CCard>
                <CCardHeader>
                  ...
                  <small className="text-body-secondary ms-2">Not closed or fulfilled</small>
                </CCardHeader>
                <CCardBody>
                  <CTable hover responsive>
                    <CTableHead>
                    ...
                    <CTableBody>
                      {dashboard.pendingQuotations.length === 0 ? (
                        ...
                        dashboard.pendingQuotations.map((row) => (
                          <CTableRow key={row.id}>
                            <CTableDataCell>{row.quotationCode || '—'}</CTableDataCell>
                            <CTableDataCell>{row.companyName || '—'}</CTableDataCell>
                            <CTableDataCell>{formatQuotationStatus(row.status)}</CTableDataCell>
                            <CTableDataCell>{formatDateTime(row.createdAt)}</CTableDataCell>
                            <CTableDataCell className="text-end">
                              <CButton
                                color="primary"
            ...
            <CCol lg={12} className="mb-4">
              <CCard>
                <CCardHeader>
                  ...
                  <small className="text-body-secondary ms-2">Not fulfilled or cancelled</small>
                </CCardHeader>
                <CCardBody>
                  <CTable hover responsive>
                    <CTableHead>
                    ...
                    <CTableBody>
                      {dashboard.pendingPurchaseOrders.length === 0 ? (
                        <CTableRow>
                          <CTableDataCell colSpan={6} className="text-body-secondary text-center py-4">
                        ...
                        dashboard.pendingPurchaseOrders.map((row) => (
                          <CTableRow key={row.id}>
                            ...
                            <CTableDataCell>{row.companyName || '—'}</CTableDataCell>
                            <CTableDataCell>{formatPoStatus(row.status)}</CTableDataCell>
                            <CTableDataCell>{formatAmount(row.remainingAmount)}</CTableDataCell>
                            <CTableDataCell>{formatDateTime(row.createdAt)}</CTableDataCell>
                            <CTableDataCell className="text-end">
                              {row.quotationId ? (
                                <CButton
            ...
            <CCol>
              <CCard className="mb-4">
                <CCardHeader>
                  ...
                  <small className="text-body-secondary ms-2">Branch employee targets vs billing</small>
                </CCardHeader>
                <CCardBody>
                  <CRow>
                    <CCol md={3}>
                      <div className="border-start border-start-4 border-start-success py-1 px-3 mb-3">
                        ...
                        <div className="fs-5 fw-semibold">{formatAmount(dashboard.weeklyTarget)}</div>
                        <div className="small text-body-secondary mt-1">
                          {targetPeriodLabel(dashboard.weeklyPeriodFrom, dashboard.weeklyPeriodTo)}
                        </div>
                      </div>
                    </CCol>
                    <CCol md={3}>
                      <div className="border-start border-start-4 border-start-success py-1 px-3 mb-3">
                        ...
                        <div className="fs-5 fw-semibold">{formatAmount(dashboard.weeklyBilling)}</div>
                        <div className="small text-body-secondary mt-1">
                          {targetPeriodLabel(dashboard.weeklyPeriodFrom, dashboard.weeklyPeriodTo)}
                        </div>
                      </div>
                    ...
                    <CCol md={3}>
                      <div className="border-start border-start-4 border-start-warning py-1 px-3 mb-3">
                        ...
                        <div className="fs-5 fw-semibold">{formatAmount(dashboard.monthlyTarget)}</div>
                        <div className="small text-body-secondary mt-1">
                          {targetPeriodLabel(dashboard.monthlyPeriodFrom, dashboard.monthlyPeriodTo)}
                        </div>
                      </div>
                    </CCol>
                    <CCol md={3}>
                      <div className="border-start border-start-4 border-start-warning py-1 px-3 mb-3">
                        ...
                        <div className="fs-5 fw-semibold">{formatAmount(dashboard.monthlyBilling)}</div>
                        <div className="small text-body-secondary mt-1">
                          {targetPeriodLabel(dashboard.monthlyPeriodFrom, dashboard.monthlyPeriodTo)}
                        </div>
                      </div>
