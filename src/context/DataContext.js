...
const initialQueries = [
  {
    ...
    description: 'We are interested in purchasing 50 industrial motors for our new plant. Please share the quotation.',
    images: [],
    status: 'new',
    priority: 'high',
    createdAt: new Date().toISOString(),
  },
  {
    ...
    description: 'Need 200 steel pipes for construction project. Need delivery within 2 weeks.',
    images: [],
    status: 'in_progress',
    priority: 'normal',
...
export const DataProvider = ({ children }) => {
  ...
  const getBranchesByCompany = (companyId) => branches.filter((b) => b.companyId === companyId)
  ...
  const updateBranchUser = (id, data) => {
    setBranchUsers(branchUsers.map((u) => (u.id === id ? { ...u, ...data } : u)))
  }
  ...
  const getUsersByBranch = (branchId) => branchUsers.filter((u) => u.branchId === branchId)
  ...
  const updatePurchaseOrder = (id, data) => {
    setPurchaseOrders(purchaseOrders.map((o) => (o.id === id ? { ...o, ...data } : o)))
  }
