...
export const GSTIN_OPTIONAL_PATTERN = /^(|[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z])$/
...
export const stringRequired = (min = 1, max = 255) =>
  yup.string().trim().required(MSG.required).min(min, MSG.minLength(min)).max(max, MSG.maxLength(max))

/** Optional string, max length */
export const stringOptional = (max = 255) =>
  yup.string().trim().optional().nullable().transform((v, o) => (o === '' ? null : v)).max(max, MSG.maxLength(max))
...
export const emailOptional = () =>
  yup.string().trim().optional().nullable().transform((v, o) => (o === '' ? null : v)).email(MSG.email)
