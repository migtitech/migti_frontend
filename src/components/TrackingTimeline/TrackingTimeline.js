...
import { cilUser, cilClock, cilEnvelopeClosed, cilPhone, cilPencil, cilCheckAlt } from '@coreui/icons'
...
const getActivityLabel = (type) => {
  switch (type) {
    ...
    case 'action': return 'Action Recorded'
...
const getTimestamp = (act) => act.createdAt || act.created_at || act.timestamp || null
      ...
      <CCardBody>
        ...
        {loading ? (
          ...
          activities.map((act, index) => {
            const performer = getPerformerInfo ? getPerformerInfo(act) : { name: null, email: null, phone: null, role: null }
                  ...
                  {act.type === 'follow_up' && (
                    <div className="mt-2 p-2 bg-light rounded small">
                      ...
                      {(act.meta?.note || act.metadata?.note) && (
                        <div>
                          <strong>Note:</strong> {act.meta?.note || act.metadata?.note}
                        </div>
                      )}
                    </div>
                  )}
